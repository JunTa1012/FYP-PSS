<x-app-layout>
    <div class="grid grid-cols-3 gap-12 mx-10 my-6">
            <div class="flex justify-between items-center w-auto">
                <p class="font-bold text-md">Add Recycle Activity</p>
            </div>
    </div>
    <div class="mt-2 mx-4">
        <form action="{{ route('create.recycle.activity') }}" method="POST">
            @csrf
            <div class="flex space-x-6 mt-4">
                <!-- Recycle Category -->
                <div class="flex flex-col w-1/2">
                    <label for="recycle_category" class="font-semibold text-sm text-gray mb-2">Recycle Category</label>
                    <x-input name="recycle_category" type="text" placeholder="" value="{{ old('recycle_category') }}" required/>
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <x-input name="recycle_datetime" type="datetime-local" value="{{ old('recycle_datetime') }}" required/>
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <select name="recycle_status" id="recycle_status" required class="border border-gray-300 rounded-md p-2">
                        ...
                        @foreach($recycleStatus as $status)
                            <option value="{{ $status }}" {{ old('recycle_status') == $status ? 'selected' : '' }} class="font-semibold">{{ $status }}</option>
                        @endforeach
                    </select>
            ...
            <div class="flex space-x-6 mt-4">
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <textarea name="recycle_comment" rows="3" type="text" placeholder="" required class="bg-white text-gray font-semibold border border-slate-200 focus:border-0 focus:ring-primary-400 focus:ring-1 focus:text-gray-700 rounded-md shadow-sm placeholder:text-sm placeholder:text-slate-400 placeholder:text-opacity-60">{{ old('recycle_comment') }}</textarea>
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <x-input name="recycle_weight" type="number" placeholder="" value="{{ old('recycle_weight') }}" required id="recycle_weight" oninput="calculatePrice()"/>
            ...
            <div class="flex space-x-6 mt-4">
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <x-input name="recycle_rate" type="text" placeholder="" value="{{ old('recycle_rate') }}" required id="recycle_rate" oninput="calculatePrice()"/>
                    @error('recycle_rate')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <x-input name="recycle_price" type="number" placeholder="" value="{{ old('recycle_price') }}" id="recycle_price" readonly/>
                    @error('recycle_price')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="flex flex-col w-1/2">
                    ...
                    <x-input name="reward_point_earned" type="number" placeholder="" value="{{ old('reward_point_earned') }}" id="reward_point_earned" readonly/>
